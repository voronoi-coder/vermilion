...
static bool get_targa_format_type_and_size(const targa_header &header, GLenum &format, GLenum &type, int &size) {
    ...
    // why bits_per_pixel is null?
    std::cout << "bits_per_pixel:" << header.image_spec.bits_per_pixel << '\n';
    std::cout << "alpha_depth:" << header.image_spec.alpha_depth << '\n';
...
unsigned char *load_targa(const char *filename, GLenum &format, int &width, int &height) {
    ...
    f = fopen(filename, "rb");
    if(!f){
        std::cerr << "Unable to open file:" << f <<"; errno:" << errno <<std::endl;
        return 0;
    }
