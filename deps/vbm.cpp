...
void VBObject::render(unsigned int frameIndex, unsigned int instances) {
    if(frameIndex >= m_header.num_frames)
        return;
    ...
    if(instances) {
        if(m_header.num_indices) {
            glDrawElementsInstanced(GL_TRIANGLES, m_frame[frameIndex].count, GL_UNSIGNED_INT, 
                    (GLvoid *)(m_frame[frameIndex].first * sizeof(GLuint)), instances);
        } else {
            glDrawArraysInstanced(GL_TRIANGLES, m_frame[frameIndex].first, m_frame[frameIndex].count, instances);
        }
    } else {
        if(m_header.num_indices) {
            glDrawElements(GL_TRIANGLES, m_frame[frameIndex].count, GL_UNSIGNED_INT, 
                    (GLvoid *)(m_frame[frameIndex].first * sizeof(GLuint)));
        } else {
            glDrawArrays(GL_TRIANGLES, m_frame[frameIndex].first, m_frame[frameIndex].count);
        }
    }
